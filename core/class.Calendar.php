...
class Calendar {
	...
	/*
	 ...
	 represents Sunday.
	 */
	private $dayNames = array("D", "S", "T", "Q", "Q", "S", "S");
	...
	private $dateLink = null;

	private $dateLinkType = null;
	...
	/*
	 Insert the default date link. Utilize %d, %m and %y for day, month and yaer values.
	 */
	function setDateLink($link, $type="") {
		$this->dateLink = $link;
		$this->dateLinkType = $type;
	}

	function getDateLinkType() {
	...
	function getDateLink($day, $month, $year) {
		$link = "";
		if ($this->dateLink != null) {
			$s = array("%d","%m","%y");
			$r = array($day,$month,$year);
			$link = str_replace($s,$r,$this->dateLink);
		}
		return $link;
	}
	...
	*********************************************************************************/
	...
	function getMonthHTML($m, $y, $showYear = 1) {
		...
		$s .= "<tr>\n";
		$s .= "<th align=\"center\" valign=\"top\" class=\"calendarHeader\">" . $this->dayNames[($this->startDay)%7] . "</th>\n";
		$s .= "<th align=\"center\" valign=\"top\" class=\"calendarHeader\">" . $this->dayNames[($this->startDay+1)%7] . "</th>\n";
		$s .= "<th align=\"center\" valign=\"top\" class=\"calendarHeader\">" . $this->dayNames[($this->startDay+2)%7] . "</th>\n";
		$s .= "<th align=\"center\" valign=\"top\" class=\"calendarHeader\">" . $this->dayNames[($this->startDay+3)%7] . "</th>\n";
		$s .= "<th align=\"center\" valign=\"top\" class=\"calendarHeader\">" . $this->dayNames[($this->startDay+4)%7] . "</th>\n";
		$s .= "<th align=\"center\" valign=\"top\" class=\"calendarHeader\">" . $this->dayNames[($this->startDay+5)%7] . "</th>\n";
		$s .= "<th align=\"center\" valign=\"top\" class=\"calendarHeader\">" . $this->dayNames[($this->startDay+6)%7] . "</th>\n";
		$s .= "</tr>\n";
		...
		while ($d <= $daysInMonth) {
			...
			for ($i = 0; $i < 7; $i++) {
				...
				$s .= ($this->getDateLinkType() != "") ? " " . $this->getDateLinkType() . '="'.  $this->getDateLink($d, $month, $year) .'"' : "";
				$s .= ">";
				if ($d > 0 && $d <= $daysInMonth) {
					$link = $this->getDateLink($d, $month, $year);
					$s .= (($link == "" || $this->getDateLinkType() != "") ? $d : "<a href=\"$link\">$d</a>");
				} else {
					$s .= "&nbsp;";
				}
				$s .= "</td>\n";

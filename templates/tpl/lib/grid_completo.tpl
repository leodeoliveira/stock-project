...
{/if}
	...
	{if $paginas}
	<div id="barra_navegacao_{$cdTela}" class="barra_navegacao">

		<div id="ctrl_esq_{$cdTela}" style="float:left;">
			<span onclick="xpaginacao.primeira('{$cdTela}','{$objJS}');" class="pg_primeira" id="pg_primeira_{$cdTela}">&nbsp;</span>
			<span onclick="xpaginacao.anterior('{$cdTela}','{$objJS}');" class="pg_anterior" id="pg_anterior_{$cdTela}">&nbsp;</span>
		</div>
		...
		<div id="nrPaginasTop_{$cdTela}" name="nrPaginasTop_{$cdTela}" class="ConteinerNrPaginas" style="width: {if ($nr_paginas > 10)}340{else}$nr_paginas * 34{/if}px;">
			{section name=linha loop=$paginas}
				<span onclick="xpaginacao.pagina({$paginas[linha]},'{$cdTela}','{$objJS}');" onmouseover="xpaginacao.sinaliza_pagina(this);" onmouseout="xpaginacao.sinaliza_pagina(this);" class="{if $paginas[linha] == 1}nr_pagina_sel{else}nr_pagina{/if}" id="pg_{$paginas[linha]}_top_{$cdTela}" style="float: left;">{$paginas[linha]}</span>
			{/section}
		</div>
		...
		<div id="ctrl_dir_{$cdTela}" style="float:left;">
			...
			<span onclick="xpaginacao.proxima('{$cdTela}','{$objJS}');" class="pg_proxima" id="pg_proxima_{$cdTela}">&nbsp;</span>
			<span onclick="xpaginacao.ultima('{$cdTela}','{$objJS}');" class="pg_ultima" id="pg_ultima_{$cdTela}">&nbsp;</span>
			&nbsp;&nbsp;
			Ir para:&nbsp;<input name="nr_pagina_atual_old_{$cdTela}" id="nr_pagina_atual_old_{$cdTela}" type="hidden" value="1" readonly /><input id="nr_pagina_atual_{$cdTela}" type="text" maxlength="4" class="pg_carregada" onBlur="xpaginacao.vaipara(this.value,'{$cdTela}','{$objJS}');" value="1" onKeyPress="return app.apenas_inteiros(event, this);">&nbsp;/&nbsp;<span id="nr_pagina_total_{$cdTela}">{$nr_paginas}</span>
		</div>
	</div>
	{/if}
	...
	{/if}
		<table border="0" align="center" cellpadding="1" cellspacing="1" class="sortable grid" id="{$cdTela}_grid" width="100%">
			...
			{section name=col loop=$cabecalho.ds}
				{if $ordenacao > 1}
				<th id="{$cdTela}_{$smarty.section.col.index}" onclick="xgrid.ts_resortTable(this, '{$smarty.section.col.index}','{$cdTela}','{$objJS}','{$cabecalho.ord[col]}');return false;" class="th_titulo">&nbsp;&nbsp;&nbsp;{$cabecalho.ds[col]}<span id="spanord_{$cdTela}_{$smarty.section.col.index}" class="sortarrow">&nbsp;&nbsp;&nbsp;</span></th>
				{else}
